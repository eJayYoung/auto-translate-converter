...
function generateChnArr(files) {
  ...
  utils.getFilePathByName('zh-cn.js', (single_zh_cn_map, relative_path) => {
    const prefix = (`${config.prefix ? `${config.prefix}.` : ''}${relative_path.split('/').slice(2, -2).join('.')}.`).replace(/\.\./g, '.');
    for (const key in single_zh_cn_map) {
      if (JSON.stringify(i18n_zh_cn_map).indexOf(`:"${single_zh_cn_map[key]}"`) === -1) {
        i18n_zh_cn_map[`${prefix}.${key}`] = single_zh_cn_map[key];
      }
    }
  })
  utils.getFilePathByName('en.js', (single_en_map, relative_path) => {
    ...
    for (const key in single_en_map) {
      i18n_en_map[`${prefix}.${key}`] = single_en_map[key];
    }
  })
  ...
  // 整合i18n里zh-cn.js的文案
  const resArr = chnArr.filter(n => !i18nZhcnArr.some(m => m.indexOf(n[1]) !== -1)).concat(i18nZhcnArr);
  resArr.map(n => {
