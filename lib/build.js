...
function generateChnArr(files) {
  ...
  // console.log(i18n_all_zh_cn_map);

  Object.keys(i18n_all_zh_cn_map).forEach((key) => {
    if (i18n_all_zh_cn_map[key]) {
      let pinyinKey = generatePinyinKey(i18n_all_zh_cn_map[key].replace(symbolReg, ''), i18n_all_zh_cn_map[key].length);
  ...
  resArr.map(n => {
    ...
    let pinyinKey = n[1] ? generatePinyinKey(n[1].replace(symbolReg, ''), n[1].length) : '';
